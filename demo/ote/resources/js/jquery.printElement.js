...
(function(window, undefined) {
  ...
  $.fn["printElement"]["defaults"] = {
    ...
    iframeElementOptions: {
      styleToAdd:
        "border:none;position:absolute;width:0px;height:0px;bottom:0px;left:0px;", //style attributes to add to the iframe element
  ...
  function _getMarkup(element, opts) {
      ...
      if (opts["overrideElementCSS"].length > 0) {
        for (var x = 0; x < opts["overrideElementCSS"].length; x++) {
          ...
          if (typeof current == "string")
            html.push(
              '<link type="text/css" rel="stylesheet" href="' + current + '" >'
            );
          else
            html.push(
    ...
    html.push(
      '</head><body style="' +
        opts["printBodyOptions"]["styleToAdd"] +
        '" class="' +
        opts["printBodyOptions"]["classNameToAdd"] +
        '">'
    );
    html.push(
      '<div class="' + $element.attr("class") + '">' + elementHtml + "</div>"
    );
    html.push(
      '<script type="text/javascript">function printPage(){focus();print();' +
        (!$.browser.opera &&
        !opts["leaveOpen"] &&
        opts["printMode"].toLowerCase() == "popup"
          ? "close();"
          : "") +
        "}</script>"
    );
